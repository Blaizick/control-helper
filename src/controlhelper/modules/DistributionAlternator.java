...
public class DistributionAlternator 
{
    ...
    public void Init()
    {
        Events.on(PlayerPlansChangeEvent.class, e -> 
        {
            ...
            if (e.added == null || e.added.size == 0 || e.added.indexOf(p -> !p.breaking) == -1) return;

            var tmp = ArrayUtils.Copy(e.added);
            ArrayUtils.RemoveAll(tmp, p -> p == null || p.block == null || p.breaking);
            if (tmp.size < 5) return;
            if (tmp.indexOf(p -> p.block != tmp.first().block || !replaceBlocks.contains(p.block)) != -1) return;
            if (GetEdges(tmp).size > 2) return;
    ...
    {
        if (plans == null || plans.size == 0) return plans;
        BuildPlan edge = GetEdges(plans).first();

        if (edge == null) return plans;
        ...
        while (iter <= max) 
        {
            if (cur.block == null) break;
            boolean found = false;
            for (BuildPlan plan : GetCollisions(cur, plans))
            {
                if (last == plan) continue;
                sorted.add(plan);
                last = cur;
                cur = plan;
                found = true;
    ...
    public @Nullable Seq<BuildPlan> GetEdges(Queue<BuildPlan> plans)
    {
        ...
        for (BuildPlan plan : plans)
        {
            ...
            if (GetCollisions(plan, plans).size < 2) edges.add(plan);
        }
        return edges;
    }
    ...
    public Queue<BuildPlan> GetCollisions(BuildPlan plan, Queue<BuildPlan> plans)
    {
        ...
        if (plan == null || plan.block == null) return collisions;
        int trns = plan.block.size / 2 + 1;
        for (int x = plan.x - trns; x <= plan.x + trns; x++)
        {
            for (int y = plan.y - trns; y <= plan.y + trns; y++)
            {
                if (x < plan.x + trns && x > plan.x - trns && y < plan.y + trns && y > plan.y - trns) continue;
                if (x == plan.x - trns && y == plan.y - trns) continue;
                if (x == plan.x + trns && y == plan.y - trns) continue;
                if (x == plan.x - trns && y == plan.y + trns) continue;
                if (x == plan.x + trns && y == plan.y + trns) continue;
                var cur = GetPlanOnPos(new Vec2Int(x, y), plans);
                if (cur != null) collisions.add(cur);
            }  
        }
        return collisions;
    }

    public @Nullable BuildPlan GetPlanOnPos(Vec2Int pos, Queue<BuildPlan> plans)
    {
        if (pos == null || plans == null || plans.size == 0) return null;
        int id = plans.indexOf(p -> pos.samePos(p.x, p.y));
        if (id == -1) return null;
        return plans.get(id);
    }
}
