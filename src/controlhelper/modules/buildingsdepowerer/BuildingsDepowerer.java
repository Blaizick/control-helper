...
import controlhelper.core.requestexecutor.IUnmergableRequest;
...
public class BuildingsDepowerer {
    ...
    public void DepowerBuild(Building building) {
        for (Building b : building.power.graph.all) {
            if (!nodeTypes.contains(b.block) || !(b.block instanceof PowerNode))
                continue;
            if (!b.power.links.contains(building.pos()))
                continue;
            requestExecutor.AddRequest(new IUnmergableRequest.TileConfig(b, building.pos()));
        }
    }
    ...
    public void PowerBuild(Building building) {
        PowerNode.getNodeLinks(building.tile, building.block, building.team, other -> {
            if (building.power.links.contains(other.pos()))
                return;
            if (!nodeTypes.contains(other.block))
                return;
            requestExecutor.AddRequest(new IUnmergableRequest.TileConfig(other, building.pos()));
        });
    }
