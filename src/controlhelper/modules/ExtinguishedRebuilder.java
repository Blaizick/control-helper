    ...
    public class Selection {
        ...
        public void Update() {
            ...
            for (Building build : builds) {
                if (IsBreakPlannedOnPos(new Vec2(build.x, build.y)))
                    continue;
                brokenBlocks
                        .add(new BuildPlan(build.tileX(), build.tileY(), build.rotation, build.block, build.config()));
        ...
        public boolean IsBreakPlannedOnPos(Vec2 pos) {
            for (BuildPlan plan : Vars.player.unit().plans) {
                if (!plan.breaking)
                    continue;
                if (plan.build() != null && plan.build().within(pos, 0.1f))
                    return true;
            }
            return false;
        }
